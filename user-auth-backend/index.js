...
app.post('/api/signin', async (req, res) => {
    ...
    try {
        ...
        if (!user) return res.status(404).json({ status: "Invalid", message: "User does not exist" })
        ...
        if (passwordResult) {
            ...
            return res.status(401).json({ status: "Invalid", message: "Wrong password! Check again" })
        }
    } catch (error) {
